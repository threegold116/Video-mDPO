...
class VideomDPOTrainer(DPOTrainer):
    ...
    def concatenated_inputs(self, batch: Dict[str, Union[List, torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
        ...
        #THREEGOLD NEED CHANGE：增加image_sizes和modalities
        concatenated_batch["image_sizes"] = batch["image_sizes"] + batch["image_sizes"]
        concatenated_batch["modalities"] = batch["modalities"] + batch["modalities"]
    ...
    def concatenated_forward(
        ...
        if self.mode == "perturbation_loss":
            perturbation_model_kwargs = {
                "labels": batch["perturbation_labels"],#不是concatenated_batch
                "images": batch["image"],
                "mask_visual_tokens": False,
                "image_sizes": batch["image_size"],
                "modalities": batch["modality"],
            }
            perturbation_outputs, perturbation_label = model(
                batch["perturbation_input_ids"],
                attention_mask=batch["perturbation_attention_mask"],
                **perturbation_model_kwargs,
            )
            perturbation_logits = perturbation_outputs.logits.to(torch.float32)
            perturbation_logps = self._get_batch_logps(
                perturbation_logits,
                perturbation_label,
                average_log_prob=False,
            )
        else:
            perturbation_logps = None
            perturbation_logits = None
        
        
        
        
        #得到DPO中文本的choose,rejected,imageless_chosen的logps和logits
        return (chosen_logps, rejected_logps, imageless_chosen_logps,perturbation_logps, chosen_logits, rejected_logits, imageless_chosen_logits,perturbation_logits)
    ...
    def dpo_loss(
        ...
        policy_perturbation_logps: torch.FloatTensor,
        reference_chosen_logps: torch.FloatTensor,
        reference_rejected_logps: torch.FloatTensor,
        reference_imageless_chosen_logps: torch.FloatTensor, 
        reference_perturbation_logps: torch.FloatTensor,
        ...
        if reference_perturbation_logps!=None and self.mode == "perturbation_loss":
            perturbation_conditional_pi_logratios = policy_chosen_logps - policy_perturbation_logps
            perturbation_conditional_ref_logratios = reference_chosen_logps - reference_perturbation_logps

        if reference_free:
            image_conditional_ref_logratios = 0
            perturbation_conditional_ref_logratios = 0
        ...
        if reference_perturbation_logps!=None and  self.mode == "perturbation_loss":
            perturbation_rewards = (
                self.beta * (policy_perturbation_logps - reference_perturbation_logps).detach()
            )
        else:
            perturbation_rewards = None

        return losses, chosen_rewards, rejected_rewards, imageless_rewards,perturbation_rewards
    ...
    def get_batch_metrics(
        ...
        losses, chosen_rewards, rejected_rewards, imageless_rewards,perturbation_rewards = self.dpo_loss(
            policy_chosen_logps,
            policy_rejected_logps,
        ...
        if reference_perturbation_logps!=None and  self.mode == "perturbation_loss":
            perturbation_reward_accuracies = (chosen_rewards > perturbation_rewards).float()
        ...
        '''
        先不保留perturbation_loss的metrics
        '''
        # if reference_perturbation_logps!=None and  self.mode == "perturbation_loss":
        #     metrics[f"{prefix}logps/perturbation_chosen"] = policy_perturbation_logps.detach().cpu().mean()
        #     metrics[f"{prefix}logits/perturbation_chosen"] = perturbation_logits.detach().cpu().mean()
        #     metrics[f"{prefix}rewards/perturbation_chosen"] = perturbation_rewards.detach().cpu().mean()
        #     metrics[f"{prefix}rewards/perturbation_accuracies"] = perturbation_reward_accuracies.detach().cpu().mean()
