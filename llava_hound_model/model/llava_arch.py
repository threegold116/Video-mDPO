...
from llava_hound_model.constants import IGNORE_INDEX, X_TOKEN_INDEX, DEFAULT_X_PATCH_TOKEN, DEFAULT_X_START_TOKEN, DEFAULT_X_END_TOKEN
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_image_tower"):
            delay_load = getattr(config, "delay_load", False)#THREEGOLD NEED CHANGE: 
            self.image_tower = build_image_tower(config, delay_load=delay_load, cache_dir=cache_dir)
            self.mm_projector = build_vision_projector(config)
        if hasattr(config, "mm_video_tower"):
            delay_load = getattr(config, "delay_load", False)#THREEGOLD NEED CHANGE: 
            self.video_tower = build_video_tower(config, delay_load=delay_load, cache_dir=cache_dir)
            self.mm_projector = build_vision_projector(config)
