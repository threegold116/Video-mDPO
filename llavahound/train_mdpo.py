...
from llava_hound_model.model import *
from llavahound.modeling_llavahound import mDPOLlavaLlamaForCausalLM
from llavahound.data_collator_llavahound import mDPODataCollatorLlaVAHound
...
def train(config_dict,attn_implementation="flash_attention_2"):
    global local_rank
    ...
    model = mDPOLlavaLlamaForCausalLM.from_pretrained(
            model_args.model_name_or_path,
            config=config,
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        ...
        padding_side="right",
        use_fast=False,#THREEGOLD NEED CHANGE: follow https://github.com/haotian-liu/LLaVA/issues/661
        trust_remote_code=True,
    )
    ...
    trainer = VideomDPOTrainer(
        ...
        data_collator=mDPODataCollatorLlaVAHound(
            tokenizer=tokenizer,
            model=model,
    ...
    if model_args.unfreeze_mm_vision_tower:
        trainer.model.get_video_tower().requires_grad_(True)
        trainer.model.get_image_tower().requires_grad_(False)  
    else:
        trainer.model.get_video_tower().requires_grad_(False)  
        trainer.model.get_image_tower().requires_grad_(False)  
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        config_path = "/share/home/jfliang/Project/Hall/Video-mDPO/llavahound/config_mdpo_loss_shuffle_frames.yaml"
